...
当前实现与目标架构的关系：目标是独立的调度层 Skill；现阶段 `video-spec-director-dev` 仍临时承载默认入口和通用视频编排工作流两部分职责。本文档中的“扩展边界”表示需要逐步抽取或新增的能力，不表示已经自动可用。
...
### 4.2 当前 `video-spec-director-dev` 的职责

当前 `video-spec-director-dev` 是通用视频编排工作流的当前实现，以影片素材为主要输入，仍包含以下内部步骤：
...
- `video-spec-director-dev` 当前作为通用视频编排工作流的实现被调度；
...
其中，`video-spec-director-dev` 当前是被调度的通用视频编排工作流，以影片素材为主要输入，不是调度层，也不是特定类型生产工作流。未来新增视频类型、内容来源或平台能力时，优先通过工作流注册、共享能力复用和契约扩展接入，不改变平台级主链路。
