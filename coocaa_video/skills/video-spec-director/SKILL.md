---
name: video-spec-director
description: Use when 用户需要从视频想法、素材或已有规格编排通用分镜和 video-spec，且没有匹配的独立视频类型工作流。
---

[工作流定位]
    本 Skill 是通用视频规格编排工作流，可在任意已初始化的视频工作区中直接调用。
    它负责影片/主题研究、需求追问、叙事与剪辑规划、分镜和 `video-spec.md`。

    - 用户直接提出视频创作、修改分镜、调整节奏、字幕、配音或素材使用时，进入本 Skill。
    - 本 Skill 保留 0-1 / 迭代两种模式，不依赖外部调度层或路由上下文。
    - TTS、字幕、音频与 HyperFrames 是可调用能力，不是新的顶层工作流。
    - 生成规格后，先完成工作流内部自检并请用户确认，再进入 HyperFrames 的 lint/check/preview/render。
...
[启动检查]
    ...
    6. 本 Skill 独立处理通用视频规格编排；如用户明确要求特定视频类型或外部工作流，再按用户指定处理。
...
[工作流程]
    ...
    [工作流身份记录]
        输出 `video-spec.md` 时必须记录 `project_slug` 和 `source_film_slug`；如调用方提供 `workflow_id` / `workflow_version`，一并记录，否则留空。
        这些字段描述项目身份和可选来源，不替代视频类型、生产方式或创作内容。
