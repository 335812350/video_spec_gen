...
video-spec-director 就是来陪你过这一关的。启用之后,你在 Codex 里说一句"我想做个视频",它就接管对话,像编导听你讲 brief 那样一路追问:这视频给谁看?多长?最想让人记住哪句话?哪个镜头是重点?你答不上来的、压根没想到的地方,它会停下来提醒你、帮你补上。
...
整件事是两个 skill 接力。video-spec-director 在上游,把你的想法变成脚本;HyperFrames 在下游,把脚本变成视频。
...
```
        你:"我想做个视频"
                ...
                ▼
   ┌────────────────────────┐
   │ video-spec-director │ 追问 + 拆镜头,陪你想清楚
   └────────────────────────┘
                │
                ▼
...
所以用之前，先安装本 Skill，并确保 HyperFrames 已安装。

## 安装

这个 skill 我主要在 **Codex** 里用,Codex 是它最顺手的场景。

本 Skill 通过 `coocaa_video/skills/video-spec-director/` 分发。使用前同时安装 HyperFrames(下游负责渲染):

```bash
npx skills add heygen-com/hyperframes
```

这条命令装好后,Codex 这些环境都能调用,不用一个工具一个工具地装。
...
> 在 Codex 里,除了说人话自动触发,也可以直接调用 `video-spec-director`。
