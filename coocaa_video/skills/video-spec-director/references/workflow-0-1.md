...
本流程负责通用的视频需求采访、素材盘点、表达规划、分镜拆解和 video-spec 输出。
独立视频类型工作流由调度层直接加载，不在本流程内通过类型模块改写问题或验收规则。
本流程仍需沿用调度层传入的 `workflow_id`、`video_type` 和 `production_mode`，并将其写入统一规格。

[使用时机]
    目标交付项目的 `projects/<project-slug>/` 里没有 `video-spec.md`，用户第一次表达想做视频；若同一 `film-slug` 的其他视频类型已有项目，仍创建新的 `project-slug`。
    多影片目录和路径解析遵循 `references/project-layout.md`。

[工作流]
    Phase 1 视频基本盘 → Phase 2 素材盘点 → 内容理解与时长决策 → Phase 3 表达手段 →
...
[目标]
    让用户的视频意图收敛到具体决策——目的 / 受众 / 平台 / 时长策略 / 核心信息 / 节奏 / 调性。时长可以是固定值、软范围，也可以暂时未知。
...
[硬指标]
    ...
    - 平台与规格四连答（平台 / 比例 / 帧率已知；时长明确为固定值、软范围或内容驱动待定）
    - 核心信息一句话 ≤ 12 字
    - 信息密度三选一（hook / 教程 / 纪录）
...
[围栏]
    ...
    - 不接受 12 字以上的核心信息——砍
    - 不接受"看着办"；时长未确认时先给相对节奏和候选范围，时长确认后再给绝对值
...
[硬指标]
    ...
    - 节奏基准：时长已确认时精确到平均每镜头秒数；时长未确认时先记录快慢层级和镜头密度范围（钉秒数前必读 `references/pacing-rules.md`，
      按视频类型 / 平台查节奏档位，不许凭印象给秒数）
    - 叙事节拍曲线（hook / 展开 / 高潮 / CTA 的顺序与承载内容；精确时间区间在时长方案确认后确定）
...
[围栏]
    ...
    - 不接受"看着办的节奏"；在时长方案确认后必须钉到秒，确认前先记录相对节奏和候选范围
    - 不许诺系统做不到的能力——给替代方案
...
[硬指标]
    - 主题选定（8 个 HyperFrames 预设之一的名字，或自定义主题 = 目标项目工作目录的 design.md）
    - accent 色明确（默认 / hex）
...
[Gate 通过条件]
    - Phase 1 所有硬指标达成（时长策略可以是固定、软范围或内容驱动，不要求前置精确秒数）
    - Phase 2 所有素材状态明确
    - 已完成素材/对白/节奏信号的内容理解，并形成可审阅的叙事弧线和时长建议
...
[正式输出门槛]
    - 对 asset-led 或高光片段项目，必须先完成源片内容分析，识别可成立的叙事弧线、最小完整片段和舍弃内容。
    - 时长确认可以来自用户固定值，也可以来自内容分析后的推荐方案；不能把“约 xx 秒”直接当成最终时长。
    - 在内容分析和时长确认完成前，只输出分析结果、候选方案或临时审阅记录，不生成正式 edit-plan.md / video-spec.md。

[输出规则]
    - 按 `templates/video-spec-template.md` 的骨架输出
    ...
    - 最终分镜和时间轴时长精确到 0.1s；探索记录可以使用时长范围或 content-driven 标记
    - 输出前用 spec-rules.md 的 [交付前自检清单] 逐条过一遍
    - 素材路径先指向权威来源 `assets/<film-slug>/...`；若 HyperFrames 工程需要项目内路径，
      再暂存到 `projects/<project-slug>/assets/`，不要把暂存副本当成新的素材来源
    - 文件名 `video-spec.md`，放目标项目的 `projects/<project-slug>/`，不要写到仓库根目录

[完成后告诉用户]
    `projects/<project-slug>/video-spec.md` 已生成。项目 manifest 应填写 `source_film_slug`；输入 /hyperframes 让渲染端按这份脚本生成视频；
    新版本写入 `outputs/<project-slug>/render-vNNN.mp4`。
