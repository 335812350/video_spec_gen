...
[输入形态]
    ...
    4. 都没有
        基于核心信息 + 受众 + 时长策略，你写草稿（前置：Phase 1 五维度齐全；内容驱动项目先提出时长范围）

[输入校验]
    每种输入都必须能回答：
    - 时长策略（来自前期；可以是固定值、软范围或内容驱动待定）
    - 信息密度档位（hook / 教程 / 纪录）
    - 核心信息（≤ 12 字）
...
### [一、内容分割]
    ...
    [都没有]
        你根据 Phase 1 五维度起草大纲：
        - 若时长已确认，按节奏类型 × 总时长推算镜头数；若未确认，先按内容节点和节奏档位提出镜头数量范围
        - 起草大纲三段
        - 必须让用户确认大纲再继续拆细
        - 不要从零跳到逐字稿——中间要有大纲校准
...
### [二、节奏分配]
    ...
    [总时长校验]
        - 时长方案确认后，所有镜头时长 + 转场时长（粗略） ≈ 最终总时长
        - 转场粗估 0.2-0.5s/转场（精确值后续渲染时定）
        - 最终时间轴误差 ±0.5s 以内允许；探索阶段不对用户最初软时长做误差校验
...
### [六、素材绑定]
    ...
    [素材绑定纪律]
        ...
        - 素材路径先标注权威来源（`assets/<film-slug>/...`）；需要进入 HyperFrames 工程时再使用项目内暂存路径
