...
## 时长决策分层
- 探索阶段允许记录 `duration: content-driven`、`duration_hint` 或 `duration_range`，用于表示内容分析尚未完成或用户只给了软偏好。
- 正式 `video-spec.md` 必须在内容分析、叙事弧线和时长确认完成后生成，并包含明确的 `duration_s`、完整分镜时间轴和音频起止时间。
- 对 high-energy / asset-led 项目，不能为了满足初始秒数而填充空镜或截断完整事件；若用户给了目标时长，应先作为筛选偏好，再由内容审核决定是否采纳。

`templates/video-spec-template.md` 是要填的骨架，本文件是每个字段的约束、规格一致性校验、和输出前的自检清单。

通用原则：

- `workflow_id`、`workflow_version`、`project_slug`、`source_film_slug` 必须明确；它们分别对应调度结果、工作流版本、当前交付项目和共享源影片身份。
- `video_type` 与 `production_mode` 必须明确，并与调度层选择的工作流契约一致；独立工作流的专属规则由对应 Skill 维护。
...
- 时长与信息密度匹配：抓眼球型、讲清楚型和沉浸型只提供参考范围；最终时长由内容量、叙事完整性和平台约束共同决定，不把初始时长偏好当成硬目标
...
### § 1 视频基本盘
- 工作流身份字段必须与目标项目 `project.json` 一致；类型模块不能默默改写 `workflow_id`。
...
### § 2 叙事结构
- 探索阶段可以先记录叙事顺序、相对节奏和时长范围；正式 spec 阶段叙事节拍各段时间区间加起来 = 全片总时长
...
- 使用 A-roll 时必须有完整旁白原文（字幕逐词高亮，省略号不行）；纯素材工作流可不使用 A-roll
...
- [ ] `workflow_id`、`workflow_version`、`project_slug`、`source_film_slug` 已记录并与 `project.json` 一致
- [ ] 规格一致性校验全过（见上）
...
- [ ] 节奏档位与信息密度匹配；正式 selection / video-spec 已有明确总时长且时间轴误差 ≤ ±0.5s（详见 `pacing-rules.md`）
