...
[按工作流上下文裁剪]
    调度层已经选择独立工作流时，本流程只吸收其公共需求字段，不重新判断顶层类型。
    通用工作流根据目的、受众、平台、素材和生产方式裁剪后续问题；独立工作流的专属问题、
    素材策略和验收规则由对应 Skill 维护。
...
[Phase 2 · 素材盘点]
    ...
    [2.2 · 音频素材]
        ...
        [主问题]
            先告知能力："渲染端支持阿里云百炼 TTS。配置好 `.env.local` 的 Key 后，可以选模型、系统音色或复刻音色并生成 MP3；具体可用模型和音色我先查询当前 Workspace，不凭示例名猜。"
            然后问：
            - 旁白怎么搞？三个选项：
                1. **真人录音**（你或同事录，发我 wav/mp3）
                2. **阿里云 TTS**（模型 + voice ID，由当前 Workspace 授权决定）
                3. **不要旁白**（纯文字 + 音乐）
        ...
        [追问深化]
            **(a) TTS 路径**
                - 用户说"用 AI 配音"→ 你回："我们走阿里云 TTS。先确认模型、系统音色还是复刻音色，以及是否需要 SSE；我会先查当前账号能用的模型和 voice ID。"
                - 用户说"中文配音"→ 你回："可以按旁白风格试听阿里云候选模型和音色。你更看重自然度、情绪表现，还是速度和成本？"
                - 用户说"用 GPT 那种 voice"→ 你回："阿里云的 voice ID 与模型绑定。你要用系统音色，还是提供一段有授权的录音做复刻？"
                - 用户说"语速正常"→ 你回："我会先按模型默认速度生成；如果要更慢或更有播音感，请把语速/情绪写进配音参数，必要时用 `--extra-json` 传模型支持的字段。"
        ...
        [接受标准]
            - 旁白路径确定：真人 / 阿里云 TTS（模型 + voice ID，或复刻音色）/ 无旁白
    ...
    [2.4 · 图形素材]
        ...
        [追问深化]
            ...
            **(c) 字体**
                ...
                - 用户说"我没有字体"→ 你回："等下选完主题就用主题自带字体（每个主题字体不同，预设由 HyperFrames 的 visual-styles.md 定义、自定义由目标项目工作目录的 design.md 定义）。除非你有强 brand 需求，否则跟主题走。"
...
[Phase 4 · 视觉主题选定 + 微调]
    ...
    [4.1 · 主题选择]
        ...
        [主问题]
            ...
            路径 2：自己的自定义主题 —— 目标项目工作目录的一个 design.md
        ...
        [路径 2 · 自定义主题]
            落地位置：目标项目 `projects/<project-slug>/` 工作目录的 `design.md`（路径基准 =
            video-spec.md 所在目录）。不再有 styles/ 文件夹。

            两种入口：
            (a) 已有设计文件 → 用户把自己的 design.md（HyperFrames YAML 格式）放目标项目工作目录 / 粘贴聊天
            (b) 只有感觉没有具体值 → 用户描述给你（三个形容词/参考链接/类似品牌），
                你上网调研后给 2-3 个候选方案，定稿后在目标项目工作目录生成 design.md
