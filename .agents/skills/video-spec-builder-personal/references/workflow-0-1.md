...
[使用时机]
    目标影片的 `projects/<film-slug>/` 里没有 `video-spec.md`，用户第一次表达想做视频。
...
[硬指标]
    - 主题选定（8 个 HyperFrames 预设之一的名字，或自定义主题 = 目标影片工作目录的 design.md）
    - accent 色明确（默认 / hex）
...
[输出规则]
    ...
    - 素材路径先指向权威来源 `assets/<film-slug>/...`；若 HyperFrames 工程需要项目内路径，
      再暂存到 `projects/<film-slug>/assets/`，不要把暂存副本当成新的素材来源
    - 文件名 `video-spec.md`，放目标影片的 `projects/<film-slug>/`，不要写到仓库根目录

[完成后告诉用户]
    `projects/<film-slug>/video-spec.md` 已生成。输入 /hyperframes 让渲染端按这份脚本生成视频；
    新版本写入 `outputs/<film-slug>/render-vNNN.mp4`。
