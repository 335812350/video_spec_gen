...
内容驱动项目先从内容完整性和节奏档位提出时长范围；只有时长方案确认后，才按最终时长反推镜头数 + 转场密度。
...
[时长推算公式]
    镜头数 N = 总时长 T ÷ 平均镜头时长（T 在最终时长确认后取得）
    转场总时长 ≈ N × 0.2-0.5s（粗估）
    实际镜头总时长 = T − 转场总时长
    误差 ±0.5s 允许。
...
□ 最终总时长 = Σ镜头时长 + Σ转场时长（误差 ±0.5s；探索阶段不做目标时长误差校验）
