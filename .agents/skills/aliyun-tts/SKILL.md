...
- 实现入口是 `tools/aliyun_tts.py`，用它执行 `models`、`voices`、`synthesize` 和 `clone`。
...
- 仓库根目录 `.env.local` 保存 `DASHSCOPE_API_KEY`；可选 `DASHSCOPE_BASE_URL`。优先级是显式 CLI 参数 > 环境变量 > `.env.local`。
...
- `--voice` 必须显式提供。先用 `python tools/aliyun_tts.py models` 查询授权，再查自定义音色：Qwen-Audio/CosyVoice 通常用 `voices --model voice-enrollment`，Qwen3-TTS 用 `voices --model qwen-voice-enrollment`；可追加 `--target-model <model>` 精确筛选。不要凭文档示例猜音色是否属于账号。
...
python tools/aliyun_tts.py synthesize --model <model> --voice <voice> --text "..." --out outputs/voice.mp3 --format mp3 --sample-rate 24000
python tools/aliyun_tts.py synthesize --model <model> --voice <voice> --script script.txt --out outputs/voice.mp3 --stream
